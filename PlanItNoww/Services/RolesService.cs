...
namespace PlanItNoww.Services
{
    public class RolesService
    {
        ...
        public async Task<List<Roles>> SelectTransaction(IDb db, RolesSelectReq req)
        {
            ...
            string query = @"
                SELECT id, name, bitmask, version, notes, createdby, createdon, modifiedby, modifiedon, 
        ...
        public async Task InsertTransaction(IDb db, Roles roles)
        {
            String query = @"
                INSERT INTO Roles (
                    name, bitmask, version, notes, createdby, createdon, modifiedby, modifiedon, 
                    attributes, isactive, issuspended
                )
                VALUES (
                   @name, @bitmask, @version, @notes, @createdby, @createdon, @modifiedby, @modifiedon, 
            ...
            roles.createdby = requeststate.usercontext.userid;
            roles.modifiedon = DateTime.UtcNow;
            roles.modifiedby = requeststate.usercontext.userid;
            ...
            db.AddParameter(command, "attributes", DbTypes.Types.Json).Value = roles.attributes_json;
        ...
        public async Task<bool> UpdateTransaction(IDb db, Roles roles)
        {
            ...
            String query = @"
                UPDATE Roles
                SET 
                    name = @name, bitmask = @bitmask, version = @version, notes = @notes, 
