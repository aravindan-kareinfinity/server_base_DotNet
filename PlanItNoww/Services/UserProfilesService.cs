...
namespace PlanItNoww.Services
{
    public class UserProfilesService
    {
        ...
        public async Task<List<UserProfiles>> SelectTransaction(IDb db, UserProfilesSelectReq req)
        {
            ...
            string query = @"
                SELECT id, userid, fullname, dob, gender, address, city, state, zipcode, country, 
                       profileimageurl, version, notes, createdby, createdon, modifiedby, modifiedon, 
                       attributes, isactive, issuspended
                FROM UserProfiles
                ";
        ...
        public async Task InsertTransaction(IDb db, UserProfiles userprofiles)
        {
            String query = @"
                INSERT INTO UserProfiles (
                    userid, fullname, dob, gender, address, city, state, zipcode, country, 
                    profileimageurl, version, notes, createdby, createdon, modifiedby, modifiedon, 
                    attributes, isactive, issuspended
                )
                VALUES (
            ...
            userprofiles.createdby = requeststate.usercontext.userid;
            userprofiles.modifiedon = DateTime.UtcNow;
            userprofiles.modifiedby = requeststate.usercontext.userid;
            ...
            db.AddParameter(command, "attributes", DbTypes.Types.Json).Value = userprofiles.attributes_json;
        ...
        public async Task<bool> UpdateTransaction(IDb db, UserProfiles userprofiles)
        {
            ...
            String query = @"
                UPDATE UserProfiles
                SET 
                    userid = @userid, fullname = @fullname, dob = @dob, gender = @gender, 
                    address = @address, city = @city, state = @state, zipcode = @zipcode, 
                    country = @country, profileimageurl = @profileimageurl, version = @version, 
                    notes = @notes, createdby = @createdby, createdon = @createdon, 
                    modifiedby = @modifiedby, modifiedon = @modifiedon, attributes = @attributes, 
                    isactive = @isactive, issuspended = @issuspended,
                    version = version + 1
                WHERE id = @id
                ";
