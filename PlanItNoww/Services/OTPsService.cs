...
namespace PlanItNoww.Services
{
    public class OTPsService
    {
        ...
        public async Task<List<OTPs>> SelectTransaction(IDb db, OTPsSelectReq req)
        {
            ...
            string query = @"
                SELECT id, userid, emailormobile, otpcode, purpose, expiresat, isused, version, notes, 
        ...
        public async Task InsertTransaction(IDb db, OTPs otps)
        {
            String query = @"
                INSERT INTO OTPs (
                    userid, emailormobile, otpcode, purpose, expiresat, isused, version, notes, 
                    createdby, createdon, modifiedby, modifiedon, attributes, isactive, issuspended
                )
                VALUES (
                   @userid, @emailormobile, @otpcode, @purpose, @expiresat, @isused, @version, @notes, 
            ...
            otps.createdby = requeststate.usercontext.userid;
            otps.modifiedon = DateTime.UtcNow;
            otps.modifiedby = requeststate.usercontext.userid;
            ...
            db.AddParameter(command, "attributes", DbTypes.Types.Json).Value = otps.attributes_json;
        ...
        public async Task<bool> UpdateTransaction(IDb db, OTPs otps)
        {
            ...
            String query = @"
                UPDATE OTPs
                SET 
                    userid = @userid, emailormobile = @emailormobile, otpcode = @otpcode, purpose = @purpose, 
                    expiresat = @expiresat, isused = @isused, version = @version, notes = @notes, 
                    createdby = @createdby, createdon = @createdon, modifiedby = @modifiedby, modifiedon = @modifiedon, 
                    attributes = @attributes, isactive = @isactive, issuspended = @issuspended,
                    version = version + 1
                WHERE id = @id
                ";
