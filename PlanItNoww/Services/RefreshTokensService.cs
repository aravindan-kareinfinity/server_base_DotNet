...
namespace PlanItNoww.Services
{
    public class RefreshTokensService
    {
        ...
        public async Task<List<RefreshTokens>> SelectTransaction(IDb db, RefreshTokensSelectReq req)
        {
            ...
            string query = @"
                SELECT id, token, revoked, userid, expiresat, version, notes, createdby, createdon, 
        ...
        public async Task InsertTransaction(IDb db, RefreshTokens refreshtokens)
        {
            String query = @"
                INSERT INTO RefreshTokens (
                    token, revoked, userid, expiresat, version, notes, createdby, createdon, 
                    modifiedby, modifiedon, attributes, isactive, issuspended
                )
                VALUES (
                   @token, @revoked, @userid, @expiresat, @version, @notes, @createdby, @createdon, 
            ...
            refreshtokens.createdby = requeststate.usercontext.userid;
            refreshtokens.modifiedon = DateTime.UtcNow;
            refreshtokens.modifiedby = requeststate.usercontext.userid;
            ...
            db.AddParameter(command, "attributes", DbTypes.Types.Json).Value = refreshtokens.attributes_json;
        ...
        public async Task<bool> UpdateTransaction(IDb db, RefreshTokens refreshtokens)
        {
            ...
            String query = @"
                ...
                SET 
                    token = @token, revoked = @revoked, userid = @userid, expiresat = @expiresat, 
